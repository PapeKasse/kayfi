<?php
   ...
   if (isset($_GET['supprimer'])) {
       //permet de supprimer une literie
       require_once("../classe/classeLiterie.php");
       $Id= htmlspecialchars($_GET['supprimer']);
       $str = explode("$", $Id);
       $a=0;
       $objet = new Literie();
       foreach ($str as $ide) {
           $a= $objet->deleteLiterie($ide);
       }
       echo $a;
    ...
    $Literie = new Literie();
        ...
        and !empty($_POST['descript'])){
            ...
            if (in_array($extension, $extensions) && $size <= $maxSize && $error == 0) {
                ...
                echo $Literie->addLiterie($nom,$prix,$prixBarre,$descript,$image);
            }
        }
            else {
                echo 2;
        ...
        if(!empty($_POST['nom']) and !empty($_POST['prix']) and !empty($_POST['prixBarre']) and !empty($_POST['descript'])){
            ...
            $prixBarre = htmlspecialchars(trim($_POST['prixBarre']));
            $descript = htmlspecialchars(trim($_POST['descript']));
            $idLiterie = htmlspecialchars($_POST['modifier']);
            ...
            if (in_array($extension, $extensions) && $size <= $maxSize && $error == 0) {
                ...
                //$file = 5f586bf96dcd38.73540086.jpg
                move_uploaded_file($tmpName, '../controller/photos/'.$image);
                echo $Literie->updateLiterie($nom, $prix,$prixBarre,$descript,$image,$idLiterie);
            }
        }
            else {
                echo 2;
        ...
        if (isset($opt)) {
            ...
            $option = $opt[0];
            if ($option == "ajouter") {
                include('php/view/literie/ajouter.php');
            } elseif ($option == "modifier") {
                require_once('php/classe/classeliterie.php');

                include('php/view/literie/modifier.php');
            } elseif ($option=="supprimer") {
                require_once('php/classe/classeliterie.php');

                include('php/view/literie/liste.php');
            } elseif ($option=="details") {
                include('php/view/literie/details.php');
            } elseif ($option=="liste") {
                include('php/view/literie/liste.php');
            }
        } 
