<?php
   ...
   if (isset($_GET['supprimer'])) {
       ...
       require_once("../classe/classeDeco.php");
       $Id= htmlspecialchars($_GET['supprimer']);
       $str = explode("$", $Id);
       $a=0;
       $objet = new Deco();
       foreach ($str as $ide) {
           $a= $objet->deleteDeco($ide);
       }
       echo $a;
    ...
    $Deco = new Deco();
        ...
        and !empty($_POST['descript'])){
            ...
            if (in_array($extension, $extensions) && $size <= $maxSize && $error == 0) {
                ...
                echo $Deco->addDeco($nom,$prix,$prixBarre,$descript,$image);
            }
        }
            else {
                echo 2;
        ...
        if(!empty($_POST['nom']) and !empty($_POST['prix']) and !empty($_POST['prixBarre']) and !empty($_POST['descript'])){
            ...
            $prixBarre = htmlspecialchars(trim($_POST['prixBarre']));
            $descript = htmlspecialchars(trim($_POST['descript']));
            $idDeco = htmlspecialchars($_POST['modifier']);
            ...
            if (in_array($extension, $extensions) && $size <= $maxSize && $error == 0) {
                ...
                //$file = 5f586bf96dcd38.73540086.jpg
                move_uploaded_file($tmpName, '../controller/photos/'.$image);
                echo $Deco->updateDeco($nom, $prix,$prixBarre,$descript,$image,$idDeco);
            }
        }
            else {
                echo 2;
        ...
        if (isset($opt)) {
            ...
            $option = $opt[0];
            if ($option == "ajouter") {
                include('php/view/deco/ajouter.php');
            } elseif ($option == "modifier") {
                require_once('php/classe/classedeco.php');

                include('php/view/deco/modifier.php');
            } elseif ($option=="supprimer") {
                require_once('php/classe/classedeco.php');

                include('php/view/deco/liste.php');
            } elseif ($option=="details") {
                include('php/view/deco/details.php');
            } elseif ($option=="liste") {
                include('php/view/deco/liste.php');
            }
        } 
